...
1. 확정: 개인 프로필은 이메일 인증 직후 생성한다.
2. 확정: 기업 검증 MVP는 사업자번호 hash + 증빙 + 관리자 수동 승인이다.
3. 확정: 한 사용자의 다중 기업 소속을 허용한다.
4. 확정: V3/P1/P2 동안 LOCAL 비밀번호 해시는 `users.password_hash`에 유지하고 `user_identities`에는 secret을 저장하지 않는다.
5. 보류: 기업 직접 공고 모델은 별도 ADR에서 결정한다.
6. 보류: 법적·정산·감사 데이터 보존 기간은 제품/법무 결정 후 개인정보 파기 정책 ADR로 확정한다.

V3의 상세 컬럼, 상태, 오류 코드, backfill 및 테스트 계약은 `docs/design/member-auth-v3-contract.md`를 source of truth로 사용한다.

## 11. 다음 작업 순서

1. 완료: 운영 V2 감사와 schema-only 스냅샷 준비.
2. 완료: V3 컬럼·상태·오류·LOCAL credential 계약 확정.
3. P0 보안 게이트: 설정 기반 기본 LOCAL credential을 비활성화한다.
4. V3 Identity/Consent Foundation Flyway DDL과 마이그레이션 테스트를 구현한다.
